...
auto
Context::get_queue(const Queue queue) const
...
{
  static constexpr auto num_bindings = 4U;
  // create default descriptor pool and allocate 1 descriptor set
  const std::array<VkDescriptorPoolSize, num_bindings> poolSizes = {
    VkDescriptorPoolSize{VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, current_max_textures},
    VkDescriptorPoolSize{VK_DESCRIPTOR_TYPE_SAMPLER, current_max_samplers},
    VkDescriptorPoolSize{VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, current_max_textures},
    VkDescriptorPoolSize{VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, current_max_textures},
};
  const VkDescriptorPoolCreateInfo ci = {
    .sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO,
    .flags = VK_DESCRIPTOR_POOL_CREATE_UPDATE_AFTER_BIND_BIT,
    .maxSets = 1,
    .poolSizeCount = static_cast<std::uint32_t>(num_bindings),
    .pPoolSizes = poolSizes.data(),
};
  VK_VERIFY(vkCreateDescriptorPool(vkb_device.device, &ci, nullptr, &descriptor_pool));
  ...
  VK_VERIFY(vkAllocateDescriptorSets(vkb_device.device, &ai, &descriptor_set),
                   "Failed to allocate descriptor set");
}


  constexpr auto grow_factor = 1.5F;
  auto current_textures = current_max_textures;
  auto current_samplers = current_max_samplers;
  ...
  if (current_textures != current_max_textures ||
    ...
    if (auto err = grow_descriptor_pool(current_textures,
                                        current_samplers,
                                        current_max_acceleration_structures);
        !err.has_value()) {
  ...
  for (const auto& object : texture_pool) {
    ...
    assert((is_sampled || is_storage) &&
           "Texture must be either sampled or storage");
  ...
  for (const auto& object : sampler_pool) {
    sampler_infos.emplace_back(object ? object : dummy_sampler,
                               VK_NULL_HANDLE,
...
Context::grow_descriptor_pool(std::uint32_t new_max_textures,
                              ...
                              std::uint32_t new_max_acceleration)
  -> std::expected<void, ContextError>
{
  ...
  current_max_acceleration_structures = new_max_acceleration;
...
#define VERIFY(condition, message)                                             \
  ...
  constexpr std::array<VkDescriptorSetLayoutCreateFlags, 3> binding_flags =
...
Context::create_placeholder_resources() -> void
{
  ...
  auto image = VkTexture::create(*this,
...
auto
Context::destroy_texture(TextureHandle handle) -> void
{
  SCOPE_EXIT
  {
  ...
  pre_frame_task([&alloc = *allocator_impl, tex = texture](auto, auto) {
    alloc.deallocate_image(tex->get_image());
  });
}

auto
Context::destroy_buffer(BufferHandle handle) -> void
{
  TODO("Implement buffer destruction");
}

auto
Context::destroy_compute_pipeline(ComputePipelineHandle handle) -> void
{
  TODO("Implement compute pipeline destruction");
}

auto
Context::destroy_graphics_pipeline(GraphicsPipelineHandle handle) -> void
{
  TODO("Implement graphics pipeline destruction");
}

auto
Context::destroy_sampler(SamplerHandle handle) -> void
{
  if (!handle.valid()) {
    return;
  }
...
auto
Context::destroy_shader_module(ShaderModuleHandle handle) -> void
{
